document.addEventListener('DOMContentLoaded', function() {
    ...
    let mediaTrack = null;
    // Переменные для экспозиции
    let exposureSupported = false;
    let exposureCapabilities = null;

    // Функция для настройки экспозиции
    async function setExposure(value) {
        if (!exposureSupported || !mediaTrack) return;
        
        try {
            await mediaTrack.applyConstraints({
                advanced: [{ exposureMode: 'manual', exposureCompensation: value }]
            });
            
            // Обновляем отображаемое значение
            const currentValue = mediaTrack.getSettings().exposureCompensation;
            exposureValue.textContent = currentValue.toFixed(1);
        } catch (err) {
            console.error('Ошибка при настройке экспозиции:', err);
            alert('Не удалось настроить экспозицию: ' + err.message);
        }
    }

    // Обработчик изменения слайдера экспозиции
    exposureSlider.addEventListener('input', function() {
        setExposure(parseFloat(this.value));
    });
    ...
    async function checkCameraSupport() {
        ...
        // Не используем enumerateDevices() для предотвращения двойного запроса разрешений
        // Вместо этого проверяем только наличие API
        return true;
    }
        ...
        try {
            ...
            try {
                ...
                if (stream && stream.getVideoTracks().length > 0) {
                    ...
                    // Проверяем поддержку ручного управления экспозицией
                    if (capabilities.exposureMode && capabilities.exposureMode.includes('manual') && 
                        ...
                        const currentValue = track.getSettings().exposureCompensation;
                        exposureSlider.value = currentValue;
                        exposureValue.textContent = currentValue.toFixed(1);
                        
                        // Активируем слайдер
                        exposureSlider.disabled = false;
                    } else {
                        // Экспозиция не поддерживается
                ...
                // Проверяем, связана ли ошибка с отсутствием камеры или отказом в разрешении
                if (err.name === 'NotFoundError') {
                    console.error('No camera found');
                    handleCameraError(new Error('No camera found'));
                    return;
                } else if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
                    console.error('Camera access denied');
                    handleCameraError(new Error('Camera access denied'));
                    return;
                }
                
                // Пробуем использовать запасную камеру
                try {
                    stream = await navigator.mediaDevices.getUserMedia({
                        video: {
                            width: 640,
                    ...
                    console.error('Failed to initialize fallback camera:', fallbackErr);
                ...
                if (stream && stream.getVideoTracks().length > 0) {
                    ...
                    // Настройка элементов управления экспозицией для запасной камеры
                    mediaTrack = track;
                    exposureCapabilities = capabilities;
